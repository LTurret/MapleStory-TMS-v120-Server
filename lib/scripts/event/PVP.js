...
function setup(mapid) {
    //mapid = ((level, 0-3) (type, 0-3) (mapid)) in one string
    ...
    if (eim.getProperty("started").equals("0")) {
        ...
        if (type != 0) {
            player
                .getClient()
                .getSession()
                .write(MaplePacketCreator.getPVPPoints(parseInt(eim.getProperty("red")), parseInt(eim.getProperty("blue"))));
        }
...
function broadcastType(eim, player) {
    ...
    eim.broadcastTeamPacket(
        MaplePacketCreator.getPVPType(type, player.getTeam() == 0 ? c1 : c2, player.getTeam() + 1, !eim.getProperty("started").equals("0"), lvl),
        player.getTeam()
    );
...
function addPVPScore(eim, player, score) {
    ...
    if (expAmount > 0) {
        //
        player.gainExp(
            (expAmount * player.getClient().getChannelServer().getExpRate() * calcFluctuation(x + score, calcExpGain(player), false)) | 0,
            true,
            false,
            true
        );
...
function calcTotal(score, exp) {
    return (
        exp *
        (calcFluctuation(java.lang.Math.min(score, 500), 1, true) +
            calcFluctuation(java.lang.Math.min(score, 1000), 1, true) +
            calcFluctuation(java.lang.Math.min(score, 1500), 1, true) +
            calcFluctuation(java.lang.Math.max(score - 1500, 0), 1, true))
    );
}
...
function playerDead(eim, player) {
    player
        .getClient()
        .getSession()
        .write(MaplePacketCreator.getPVPKilled(player.getName() + " has been defeated."));
    if (parseInt(eim.getProperty("ice")) == player.getId()) {
        eim.setProperty("ice", "0");
    ...
    if (score == true || score == null) {
        ...
        for (var i = 0; i < players.size(); i++) {
            ...
            players
                ...
                .getClient()
                .getSession()
                .write(MaplePacketCreator.getPVPResult(c, exp, winningTeam + 1, players.get(i).getTeam() + 1));
        }
    }
}
...
function playerDisconnected(eim, player) {
    if (
        ...
        !eim.getProperty("started").equals("2") &&
        (eim.getPlayerCount() < eim.getProperty("started").equals("1") ? 4 : 1 || parseInt(eim.getProperty("ice")) == player.getId())
    ) {
        end(eim);
    }
...
function playerExit(eim, player) {
    ...
    if (
        eim.getPlayerCount() > 0 &&
        !eim.getProperty("started").equals("2") &&
        (eim.getPlayerCount() < eim.getProperty("started").equals("1") ? 2 : 1 || parseInt(eim.getProperty("ice")) == player.getId())
    ) {
        end(eim);
    } else {
