...
function allMonstersDead(eim) {
    ...
    if (monster_number > 19) {
        map.broadcastMessage(
            tools.MaplePacketCreator.serverNotice(6, "[The Boss Quest] Congratulations! Your team has defeated all the bosses with " + points + " points!")
        );
        ...
        map.broadcastMessage(
            tools.MaplePacketCreator.serverNotice(6, "[The Boss Quest] Your team now has " + points + " points! The next boss will spawn in 10 seconds.")
        );
...
function monsterSpawn(eim) {
    ...
    if (parseInt(eim.getProperty("monster_number")) > 14) overrideStats.setHp(mob.getHp() / 2);
    ...
    if (parseInt(eim.getProperty("monster_number")) > 14) mob.setHp(mob.getHp() / 2);
...
function beginQuest(eim) {
    var map = eim.getMapInstance(mapid, 0);
    map.broadcastMessage(
        tools.MaplePacketCreator.serverNotice(6, "[The Boss Quest] The creatures of the darkness are coming in 30 seconds. Prepare for the worst!")
    );
