...
function start() {
    scheduleNew();
    em.getChannelServer().broadcastPacket(tools.MaplePacketCreator.serverNotice(6, "[Event] Field of Judgement is now open"));
    var iter = em.getInstances().iterator();
    while (iter.hasNext()) {
        var eim = iter.next();
        if (eim.getPlayerCount() > 0) {
...
function playerDead(eim, player) {
    ...
    player
        .getClient()
        .getSession()
        .write(tools.MaplePacketCreator.serverNotice(1, "You lost with " + eim.getKillCount(player) + " points."));
}
...
function allMonstersDead(eim) {
    ...
    while (iter.hasNext()) {
        ...
        winner
            .getClient()
            .getSession()
            .write(tools.MaplePacketCreator.serverNotice(1, "You win with " + eim.getKillCount(winner) + " points. You will be warped out in 2 minutes."));
