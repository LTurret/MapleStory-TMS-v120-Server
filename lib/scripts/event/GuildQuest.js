    ...
    for (var i = 0; i < mapz.length; i++) {
        ...
        if (map != null) {
            ...
            while (iter.hasNext()) {
                var chaz = iter.next();
                if (
                    ("" + chaz.getGuildId()).equals(eim.getProperty("guildid")) &&
                    chaz.getEventInstance() != null &&
                    chaz.getEventInstance().getName().equals("GuildQuest")
                ) {
                    z.add(chaz);
                }
            }
    ...
    if (player.getName().equals(eim.getProperty("leader"))) {
        ...
        if (!em.getProperty("state").equals("0")) {
            disposePlayerBelow(
                ...
                5,
                990001100,
                "There are no longer enough players to continue the Guild Quest, meaning the remaining players shall be warped out."
            );
        }
    }
}
...
function clearPQ(eim) {
    ...
    while (iter.hasNext()) {
        // Time is automatically processed
        var chr = iter.next();
        chr.changeMap(bonusMap, bonusMap.getPortal(0));
        chr.modifyCSPoints(1, 40, true);
    }
