...
function start(mode, type, selection) {
    if (mode == -1) {
        ...
        if (status == 0) {
            ...
            qm.sendNext(
                "嚇到了嗎? 如果血量變成0，你將會遇到大麻煩。我等等會給你一顆#r羅傑的蘋果#k。 請務必收下啊。 使用後你會變得更強壯。 打開消耗欄，雙擊一下蘋果 很簡單的，按一下鍵盤的 #bI#k就能了喔！"
            );
        } else if (status == 4) {
    ...
    if (mode == -1) {
        ...
        if (status == 0) {
            if (qm.getPlayerStat("HP") < 50) {
                ...
                qm.sendNext(
                    "你看，是不是很簡單？ 你可以在右側的欄位設定#b快捷鍵#k。 哈哈，你聽不懂對吧？ 喔，每隔一段時間，血量就會恢復了。 雖然很花時間，但好好運用的話可以幫助不少的。"
                );
            }
        } else if (status == 1) {
            ...
            qm.sendNextPrev(
                "恩，我能教你的就只有這些了 我知道很難過，但是我們真的該說再見了。 好好照顧自己吧\r\n\r\n#fUI/UIWindow.img/QuestIcon/4/0#\r\n#v2010000# 3 #t2010000#\r\n#v2010009# 3 #t2010009#\r\n\r\n#fUI/UIWindow.img/QuestIcon/8/0# 10 exp"
            );
        } else if (status == 3) {
            qm.gainExp(10);
